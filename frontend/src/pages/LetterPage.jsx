...
const LetterPage = () => {
  ...
  const [cfpbEscalationError, setCfpbEscalationError] = useState(null);
  ...
  // When channel is CFPB, populate CFPB content directly from currentLetter (no API call)
  // This enforces: one enforcement engine, multiple renderers
  useEffect(() => {
    if (documentChannel === 'CFPB' && currentLetter && !cfpbLetter) {
      setCfpbLetter({ content: currentLetter.content });
      setEditableCfpbContent(currentLetter.content);
    }
  }, [documentChannel, currentLetter, cfpbLetter]);

  // Auto-expand CFPB Stage 1 when channel is CFPB and letter is ready (UI-only)
  useEffect(() => {
    if (documentChannel === 'CFPB' && currentLetter && expandedStage === 'initial') {
      setExpandedStage('cfpb1');
    }
  }, [documentChannel, currentLetter, expandedStage]);
  ...
  // NOTE: Bureau is now set by AuditPage/ViolationList BEFORE navigation
  // based on the selected violations' bureau. Do NOT auto-set from auditResult
  // because auditResult.bureau is the report-level bureau (always "transunion"
  // for multi-bureau IdentityIQ reports), not the violation-level bureau.
  ...
  // Auto-generate CFPB complaint when CFPB Stage 1 opens (MAILED channel escalation only)
  const triggerCfpbGeneration = async () => {
    // When channel is CFPB, content comes from currentLetter - no API call
    // This is only used for MAILED channel users who escalate to CFPB later
    if (documentChannel === 'CFPB') return;
  ...
  useEffect(() => {
    // Use activeDispute.id if available, otherwise use letter_id (CFPB-First route)
    ...
    if (
      ...
      (async () => {
        ...
        try {
          ...
          setCfpbEscalationError(err.message || 'Failed to generate escalation complaint');
          hasCfpbEscalationAutoGenerated.current = false; // Allow retry
        } finally {
          setCfpbEscalationLoading(false);
        }
      })();
    }
  }, [expandedStage, cfpbCase, activeDispute, currentLetter, cfpbEscalationLetter, cfpbEscalationLoading]);
            ...
            >
                          ...
                          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="flex-start">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                const sessionId = activeDispute?.id || currentLetter?.letter_id;
                                if (!sessionId) {
                                  setCfpbError('No session ID available. Please generate a letter first.');
                                  return;
                                }
                                ...
                                try {
                                  ...
                                  setCfpbError(err.message || 'Failed to mark as filed');
                                } finally {
              ...
              >
                ...
                <AccordionDetails sx={{ p: 3 }}>
                  ...
                  {/* Error state */}
                  {cfpbEscalationError && !cfpbEscalationLoading && (
                    <Alert severity="error" sx={{ mb: 2 }} onClose={() => setCfpbEscalationError(null)}>
                      {cfpbEscalationError}
                      <Button
                        size="small"
                        onClick={() => {
                          hasCfpbEscalationAutoGenerated.current = false;
                          setCfpbEscalationError(null);
                          // Trigger re-generation by toggling expandedStage
                          setExpandedStage(null);
                          setTimeout(() => setExpandedStage('cfpb2'), 100);
                        }}
                        sx={{ ml: 2 }}
                      >
                        Retry
                  ...
                  {!cfpbEscalationLetter && !cfpbEscalationLoading && !cfpbEscalationError && cfpbCase && (
                    <Box sx={{ textAlign: 'center', py: 3 }}>
              ...
              <Accordion
                ...
                onChange={() => {
                  // Legal Packet unlocked after: MAILED tier-2 complete, OR CFPB Stage 1 filed (has cfpbCase)
                  const isUnlocked = (documentChannel === 'MAILED' && stageData.final.status === 'complete') ||
                                     (documentChannel === 'CFPB' && (stageData.cfpb1.status === 'complete' || cfpbCase));
              ...
              >
                ...
                <AccordionDetails sx={{ p: 3 }}>
                  ...
                  {legalPacketError && !legalPacketLoading && (
                    <Alert severity="error" sx={{ mb: 2 }} onClose={() => setLegalPacketError(null)}>
                      {legalPacketError}
                      <Button
                        size="small"
                        onClick={() => {
                          hasLegalPacketAutoGenerated.current = false;
                          setLegalPacketError(null);
                          // Trigger re-generation by toggling expandedStage
                          setExpandedStage(null);
                          setTimeout(() => setExpandedStage('legal'), 100);
                        }}
                        sx={{ ml: 2 }}
                      >
                        Retry
